...
var tables = ['jobs_by_jobid','profiles_by_profilename'];
var lookupkeys = ['jobid','profilename'];
...
exports.handle_monitoring = function(req, res, next) {
    var apiResponse;
    var results = {"backend":{status:"OK",details:{version:webappversion}},"db":{status:"",details:{}},"workers":{status:"",details:{}}};
    // check cassandradb
    var dbdetails;
    var cql="select * from profiles_by_profilename;";
    dbclient.execute(cql, function (err, result) {
        if (err){
            if (err.name === "NoHostAvailableError"){
                results.db.status = "KO";
            } else{
        ...
        dbdetails = dbclient.getState();
        results.db.details = {"openconnections":dbdetails["_openConnections"],"inflightqueries":dbdetails["_inFlightQueries"],"version":dbdetails["_hosts"][0]["cassandraVersion"]};
